...
Instead of manually adding job every time you want to send mail you can use existing code and change only EmailTransport and Email configurations in `app.php`.
...
$queuedTasksTable->createJob('Email', $data);
```
...
protected function _notifyAdmin(Comment $comment)
{
    ...
    $QueuedTasks->createJob('CommentNotification', $data);
}
```
